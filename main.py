"""小汐旁門左道 — Teaching Monster API (Minimal) v1.1"""
import os, logging
...
app = FastAPI(title="小汐旁門左道 API", version="1.1")
...
def find_any_video():
    """Fallback: find any uploaded video"""
...
async def generate(req: GenerateRequest):
    ...
    if not video:
        # Fallback: copy any existing video to this request_id's folder
        any_video = find_any_video()
        if any_video:
            d = OUTPUT_DIR / req.request_id
            d.mkdir(parents=True, exist_ok=True)
            dest = d / f"{req.request_id}.mp4"
            import shutil
            shutil.copy2(any_video, dest)
            video = dest
            logger.info(f"🔄 Copied fallback video to {dest}")
    
    if video:
        base = BASE_URL.rstrip("/")
        logger.info(f"🎬 Serving video: {video}")
    ...
    raise HTTPException(404, f"No video for {req.request_id}. Upload first via POST /upload/{{request_id}}")
...
@app.get("/", tags=["system"])
async def root():
    return {"service": "小汐旁門左道 Teaching Monster API", "version": "1.1", "docs": "/docs"}
